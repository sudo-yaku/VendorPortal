...
class WorkPendingDetails extends React.Component {
  constructor(){
    super();
    this.state = {
      accordianFlag: false
  ...
  static propTypes = {
    ...
    isGenReadingAvbl: PropTypes.func,
  ...
  render() {
    const { isSiteDetailShown, workORderInfo,
      toggleSiteDetails, toggleSwitchDetails, isGeneratorInfo, isHvacInfo, isWodEditable, isWoPending, isListloading, attachedList
      , isLoading, savedMessage, resetInfo, isErrorMessageShown, validationMessage,
      workOrderDetailInfo, isWoloading, siteDetaisLoading, switchDetailsLoading, site, selectedWorkOrderTitle, isSwitchDetailShown, esaFlag } = this.props
      let bird_nest_activity = site.get('bird_nest_activity') ? site.get('bird_nest_activity').toJS() : null

    if (isLoading || isWoloading || siteDetaisLoading || switchDetailsLoading) {
      ...
      <div className="table-responsive vp_stepper_content">
        <table className="table sortable table-bordered text-center site-table" style={{ minHeight: "288px", "background": "#FFF" }}>
          <tbody className="vzwtable text-left">
            ...
            <tr>
              <td className="Form-group" colSpan="3"><label className="Form-label">Work Scope</label>{workOrderDetailInfo.get("work_scope")}</td>
            ...
            <tr>
              ...
              {workORderInfo.get("work_type").toLowerCase() != 'mdu' && <><td className="Form-group"><label className="Form-label">Switch Name</label>{workORderInfo.get("switch")}</td>
              {workORderInfo.get("site_type") == 'SITE' ? <td className="Form-group"><label className="Form-label">Site Name</label>{workOrderDetailInfo.get("site_name")} <img hidden={site.get("root_drive") === false} data-toggle="tooltip" title="Root Active" src={RootActive} /> <img hidden={site.get("is_donor") === false} src={VSI} width={25} /> <img hidden={bird_nest_activity?.bird_restriction === 'no'} data-toggle="tooltip" title="Bird Nest Activity" src={blackbird} width={25}/> {site?.get("osw_freeze") === true ? (<FaSnowflake style={{marginLeft: 5, color : '#00bcd4'}} title="Scheduled Freeze" />) : null}</td> : null}
              <td colSpan="2" className="Form-group"><label className="Form-label">Manager Email</label>{workOrderDetailInfo.get("mgr_email")}</td></>}
            </tr>
            <tr>
              <td className="Form-group"><label className="Form-label">Work Order Status</label>{((workORderInfo.get("workorder_status") === 'WORKPENDING' || workORderInfo.get("workorder_status") === 'WORKDECLINED') && workORderInfo.get("work_declined_count") > 0) ? 'WORKDECLINED' : workOrderDetailInfo.get("workorder_status")}</td>
            ...
            {isErrorMessageShown && (<tr><MessageBox messages={List([validationMessage])} onClear={resetInfo.bind(this)} /></tr>)}

            {savedMessage && savedMessage.get("message") && (<tr><MessageBox messages={List([savedMessage.get("message")])} onClear={resetInfo.bind(this)} className="Alert--success" iconClassName="fa-thumbs-up" /></tr>)}
          </tbody>
          <tfoot>
            <tr>
              {isHvacInfo && workOrderDetailInfo.get('work_type') == 'HVAC' ? this.props.renderHvacinfo() : null}
              <td colSpan="7">
                <WorkorderFileUpload selectedWorkOrderTitle={selectedWorkOrderTitle} meta_universalid={workORderInfo.get('meta_universalid')}
                  isWodEditable={isWodEditable} genTankReadingInput={this.props.formGenReadings.bind(this)} isSaveEnabled={isErrorMessageShown} woWorkType={workOrderDetailInfo.get('work_type')} />
              </td>
            </tr>
          </tfoot>
          <tr>
          {this.state.accordianFlag && this.props.deviceInfo.length ? 
          <td colSpan="7" className="align-middle" style={{ 'textAlign': 'center', position:'relative' }} onClick={()=>this.setState({accordianFlag: !this.state.accordianFlag})}>
            <h4>
              Device Information
            </h4>
            <img src={Arrow} style={{position: 'absolute', right: '1%', top: "25%", width: "2%", rotate:'-180deg'}}/>

          </td> : 
          <td colSpan="7" className="align-middle" style={{ 'textAlign': 'left', position:'relative' }} onClick={()=>this.setState({accordianFlag: !this.state.accordianFlag})}>
            <h4>
              Device Information : {this.props.deviceInfo.length} Devices Listed
            </h4>
           {this.props.deviceInfo.length ? <img src={Arrow} style={{position: 'absolute', right: '1%', top: "25%", width: "2%"}}/> : null }
          </td>}
        </tr>
...
export default WorkPendingDetails

...
import { Accordion, AccordionSummary, AccordionDetails, Radio, RadioGroup, TextField, FormControlLabel } from '@material-ui/core'
...
class BatteryForm extends React.Component {
  ...
  render() {
    let { vendorTechName, filesData, isReplaced, batteryModel, batteryMaker } = this.state
    const labelStyle = {"color": "black", "fontSize": "1em" }

    return (
      <div className="container">
        {this.props.selectedPM && this.props.selectedPM.PM_ITEM_STATUS == 'COMPLETED' ? <div>
          ...
          <label style={labelStyle}><h5 style={{ float: 'left' }}>Last Service Vendor</h5></label>
            ...
            <TextField
              value={this.state.lastServiceVendor}
          ...
          <label style={labelStyle}><h5 style={{ float: 'left' }}>Vendor Tech Name</h5><h5 style={{ color: 'red', float: 'left' }}>*</h5></label>
            ...
            <TextField
              value={this.state.vendorTechName}
          ...
          <hr />
            <div className="row">
              <div className='col-6'>
                <label style={{"color": "black", "fontSize": "1em", marginRight : '2vw'}}><h5 style={{ float: 'left' }}>Battery Maker</h5><h5 style={{ color: 'red', float: 'left' }}>*</h5></label>

                <TextField
                  value={this.state.batteryMaker}
                  inputStyle={{ color: '#333', fontSize: 14, fontFamily: 'Arial, Helvetica, sans-serif' }}
                  onChange={(e) => this.setState({ batteryMaker: e.target.value })} />
              </div>
              <div className='col-6'>
                <label style={{"color": "black", "fontSize": "1em", marginRight : '2vw'}}><h5 style={{ float: 'left' }}>Battery Model</h5><h5 style={{ color: 'red', float: 'left' }}>*</h5></label>

                <TextField
                  value={this.state.batteryModel}
                  inputStyle={{ color: '#333', fontSize: 14, fontFamily: 'Arial, Helvetica, sans-serif' }}
                  onChange={(e) => this.setState({ batteryModel: e.target.value })} />
              </div>
            </div>
          <div className="row">
            <div className="col-4" >
              <div style={{ marginTop: '30px' }}>
                <b>
                  <span style={{ "font-size": "2vh" }}>Battery Replaced?<span className='text-danger'>*</span></span>
                </b>
                <RadioGroup name={'battery'}
                  labelPosition="right"
          ...
          <div className="row" style={{ marginTop: '40px' }}>
            ...
            <div className="col-md-8">
              <ListOfFiles onRemoveClick={this.onAttachRemove.bind(this)} fileList={filesData} />
            </div>
            <div className="col-2"></div>
            <div className="col">
              <button type="submit"
                className="Button--primary float-right mt-2"
                disabled={!(vendorTechName.length > 0 && isReplaced.length > 0 && filesData.length > 0 && batteryMaker.length >0 && batteryModel.length > 0)}
...
export default connect(stateToProps, { ...pmActions })(BatteryForm)